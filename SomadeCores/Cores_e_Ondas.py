...
class WaveControlApp:
    ...
    def _show_fourier_equations(self, amps1, amps2, title_suffix):
        ...
        fig_eq = plt.figure(figsize=(14, 10))  # Increased height
...
y(t) = Σ [Aₙ · sin(2π · fₙ · t)]
    """
        ...
        # Wrap only the explanatory text
        wrapped_explanatory = textwrap.fill(explanatory, width=80)
        ...
        ax.text(0.05, 0.95, text_content, transform=ax.transAxes, fontsize=10,  # Reduced fontsize slightly
    ...
    def _show_hilbert_info(self, event):
        ...
        fig_hilbert = plt.figure(figsize=(12, 8))  # Increased height
    ...
    A otimização busca minimizar a diferença máxima entre os envelopes:
        ...
        # Wrap the text
        wrapped_hilbert_text = textwrap.fill(hilbert_text, width=80)
        ...
        ax.text(0.05, 0.98, wrapped_hilbert_text, transform=ax.transAxes, fontsize=10,  # Reduced fontsize
                verticalalignment='top', horizontalalignment='left',  # Mudado para 'left'
    ...
    def _mix_spectral_color(self, amps):
        """Mistura as cores sem renormalizar o resultado final."""
        weights = np.abs(amps)
        total = np.sum(weights)
        if total <= 1e-12:
            return np.array([0.0, 0.0, 0.0])
    ...
    def _draw_color_card(self, ax, amps, wave_name, stage_name, color_mode='choice1'):
        ...
        if color_mode == 'choice2':
            ...
            model_label = 'Cor espectral aproximada da mistura'
        else:
            color = self._mix_visible_color(amps)
            model_label = 'Cor RGB de visualização'

        hex_color = self._rgb_to_hex(color)
        summary = self._format_color_summary(amps)
        dominant_idx = int(np.argmax(np.abs(amps))) if np.any(np.abs(amps) > 1e-12) else None
        ...
        if dominant_idx is None:
            dominant_text = 'Faixa dominante: sem componente ativa'
        else:
            dominant_freq = self.freqs[dominant_idx]
            dominant_nm = self._frequency_to_wavelength_nm(dominant_freq)
            dominant_name = self._wavelength_to_color_name(dominant_nm)
            dominant_text = (
                f'Faixa dominante: {dominant_name} '
                f'({dominant_freq} THz | {dominant_nm:.0f} nm)'
            )

        info_text = (
            f'{wave_name}\n'
            f'{stage_name}\n'
            f'{model_label}: {hex_color}\n'
            f'{dominant_text}\n\n'
            f'{summary}'
        )
    ...
    def _draw_frequency_color_scale(self, ax, color_mode='choice1'):
        ...
        if color_mode == 'choice1':
            title = 'Barra superior alinhada ao RGB de visualização'
            xlabel = 'Modo Barra como RGB: frequência (THz) na 1ª linha | comprimento de onda equivalente (nm) na 2ª'
        else:
            title = 'Barra superior com cor espectral aproximada'
            xlabel = 'Modo Soma espectral: frequência (THz) na 1ª linha | comprimento de onda equivalente (nm) na 2ª'
    ...
    def _show_visible_colors(self, event):
        ...
        fig_colors = plt.figure(figsize=(13, 8))
        ...
        mode_description = fig_colors.text(0.5, 0.645,
            '',
            ha='center', va='center', fontsize=10, color='dimgray')

        before_title = fig_colors.text(0.26, 0.86, '', ha='center', va='center',
                                       fontsize=13, fontweight='bold', color='dimgray')
        after_title = fig_colors.text(0.74, 0.86, '', ha='center', va='center',
                                      fontsize=13, fontweight='bold', color='dimgray')
        bottom_note = fig_colors.text(0.5, 0.03,
                '',
                ha='center', va='center', fontsize=10, color='dimgray')
        ...
        def refresh_visible_colors(_event=None):
            ...
            if current_mode == 'choice1':
                mode_description.set_text('Modo Barra como RGB: a barra superior usa a mesma lógica RGB de visualização dos cartões, aproximando cores como 800 THz de tons tipo #FF00FF.')
                bottom_note.set_text('Modo Barra como RGB: foco em coerência visual na tela. A barra e os cartões usam a lógica RGB renormalizada de visualização.')
                choice1_btn.ax.set_facecolor('lightsteelblue')
                choice2_btn.ax.set_facecolor('whitesmoke')
            else:
                mode_description.set_text('Modo Soma espectral: os cartões passam a seguir a paleta espectral da barra, com mistura menos saturada e mais próxima do modelo físico.')
                bottom_note.set_text('Modo Soma espectral: foco em coerência espectral. Os cartões deixam de renormalizar a mistura e se aproximam da barra superior.')
        ...
        choice1_ax = fig_colors.add_axes([0.46, 0.91, 0.18, 0.045])
        choice1_btn = Button(choice1_ax, 'Barra como RGB',
                             color='whitesmoke', hovercolor='aliceblue')
        choice1_btn.on_clicked(set_choice1)

        choice2_ax = fig_colors.add_axes([0.65, 0.91, 0.18, 0.045])
        choice2_btn = Button(choice2_ax, 'Soma espectral',
                             color='whitesmoke', hovercolor='aliceblue')
        choice2_btn.on_clicked(set_choice2)

        refresh_ax = fig_colors.add_axes([0.84, 0.91, 0.12, 0.045])
