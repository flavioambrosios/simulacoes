...
y(t) = Σ [Aₙ · sin(2π · fₙ · t)]
    ...
    def _draw_color_card(self, ax, amps, wave_name, stage_name):
        ...
        info_text = (
            f'{wave_name}\n'
            f'{stage_name}\n'
            f'Cor RGB de visualização: {hex_color}\n'
            f'{dominant_text}\n\n'
            f'{summary}'
        )
    ...
    def _draw_frequency_color_scale(self, ax):
        """Mostra a escala de cores associada a cada frequência."""
        ax.clear()
        ax.set_title('Cor espectral aproximada por frequência', fontsize=12, fontweight='bold')
        ...
        ax.set_xlabel('Modelo espectral: frequência (THz) na 1ª linha | comprimento de onda equivalente (nm) na 2ª')

        for i, freq in enumerate(self.freqs):
            ax.add_patch(
                patches.Rectangle((i - 0.5, 0), 1, 1,
                                  facecolor=self._frequency_to_rgb(freq),
    ...
    def _show_visible_colors(self, event):
        ...
        fig_colors.text(0.5, 0.645,
                'Cartões inferiores: cor RGB de visualização obtida pela mistura das componentes selecionadas.',
        ...
        def refresh_visible_colors(_event=None):
            ...
            self._draw_frequency_color_scale(ax_scale)
            self._draw_color_card(ax_before_wave1, before1, 'Onda 1', before_label)
            self._draw_color_card(ax_after_wave1, after1, 'Onda 1', 'Após a otimização')
            self._draw_color_card(ax_before_wave2, before2, 'Onda 2', before_label)
            self._draw_color_card(ax_after_wave2, after2, 'Onda 2', 'Após a otimização')

            before_title.set_text(before_label)
            after_title.set_text('Após a otimização')
            fig_colors.canvas.draw_idle()

        refresh_ax = fig_colors.add_axes([0.82, 0.91, 0.12, 0.045])
        ...
        fig_colors.text(0.5, 0.03,
                        'Dois modelos aparecem juntos: a barra superior mostra a cor espectral aproximada de cada frequência; os cartões mostram a cor RGB de visualização resultante da mistura.',
                        ha='center', va='center', fontsize=10, color='dimgray')
