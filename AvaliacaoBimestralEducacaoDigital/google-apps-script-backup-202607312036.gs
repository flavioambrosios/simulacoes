...
function doPost(e) {
  try {
    ...
    const studentRow = findOrCreateStudentRow(targetSheet, payload.estudante);
    const avaliacaoHistorico = payload.categoria === 'simulacao'
      ? ''
      : (payload.avaliacao || 'Avaliação Bimestral - Educação Digital');
    ...
    appendHistory(spreadsheet, {
      ...
      nota: payload.nota || 0,
      notaConclusaoIa: payload.notaConclusaoIa || payload.nota_conclusao_ia || '',
...
function findTargetSheet(spreadsheet, payload) {
  const targetSheetName = resolveTargetSheetName(spreadsheet, payload);
  if (targetSheetName) {
...
function resolveTargetSheetName(spreadsheet, payload) {
  ...
  for (let index = 0; index < explicitCandidates.length; index += 1) {
    const candidateName = String(explicitCandidates[index] || '').trim();
    if (candidateName && spreadsheet.getSheetByName(candidateName)) {
      return candidateName;
    }
  }

  const normalizedSeries = normalizeText(payload.serie);
  const normalizedTurma = normalizeText(payload.turma);
  const normalizedTrilha = normalizeText(payload.trilha);
  const sheets = spreadsheet.getSheets();
  ...
  for (let index = 0; index < sheets.length; index += 1) {
    ...
    const seriesTurmaMatch = extractSerieTurmaFromSheetName(sheetName);

    if (normalizedTrilha && normalizedSheetName === normalizedTrilha) {
      return sheetName;
    }

    if (normalizedSeries && normalizedTurma && seriesTurmaMatch) {
      if (normalizeText(seriesTurmaMatch.serie) === normalizedSeries && normalizeText(seriesTurmaMatch.turma) === normalizedTurma) {
        return sheetName;
      }
    }
  }
...
function appendHistory(spreadsheet, data) {
  ...
  ensureHistorySheetHeaders(historySheet);
...
function getHistorySheetHeaders() {
  return [
    'timestamp',
    'avaliacao',
    'atividade',
...
function ensureHistorySheetHeaders(historySheet) {
  const headers = getHistorySheetHeaders();
  const currentColumnCount = Math.max(historySheet.getLastColumn(), headers.length, 1);
  const currentHeaders = historySheet.getRange(1, 1, 1, currentColumnCount).getDisplayValues()[0] || [];

  headers.forEach(function(header, index) {
    if (String(currentHeaders[index] || '').trim() !== header) {
      historySheet.getRange(1, index + 1).setValue(header);
    }
  });
}
