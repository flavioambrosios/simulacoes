...
const SCRIPT_VERSION = '2026-07-31-a';
...
function sendConfirmationEmailIfPossible(payload) {
  ...
  const dedupKey = buildConfirmationEmailDeduplicationKey(payload);
  const properties = PropertiesService.getScriptProperties();
  const now = Date.now();
  const dedupWindowMs = 15 * 60 * 1000;
  const lastSentAt = Number(properties.getProperty(dedupKey) || '0');

  if (lastSentAt && (now - lastSentAt) < dedupWindowMs) {
    return { status: 'skipped', reason: 'duplicado_recente', dedupKey: dedupKey };
  }

  const lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) {
    return { status: 'skipped', reason: 'bloqueio_temporario', dedupKey: dedupKey };
  }

  try {
    const latestSentAt = Number(properties.getProperty(dedupKey) || '0');
    if (latestSentAt && (now - latestSentAt) < dedupWindowMs) {
      return { status: 'skipped', reason: 'duplicado_recente', dedupKey: dedupKey };
    }

    const payloadMessage = [
      payload.mensagem || payload.message || payload.textoEmail || '',
      '',
      '---',
      'Resumo do sistema:',
      'Estudante: ' + (payload.estudante || ''),
      ...
      'CONCLUSÃO:',
      String(payload.conclusao || payload.detalhes || ''),
      'Data de envio: ' + (payload.dataEnvio || payload.data_envio || payload.timestamp || '')
    ...
    properties.setProperty(dedupKey, String(now));
    return { status: 'sent', to: email, dedupKey: dedupKey };
  } catch (error) {
    return { status: 'failed', reason: formatRuntimeError(error), dedupKey: dedupKey };
  } finally {
    if (lock.hasLock()) {
      lock.releaseLock();
    }
...
function buildConfirmationEmailDeduplicationKey(payload) {
  const normalizedEmail = String(payload.email || '').trim().toLowerCase();
  const normalizedStudent = normalizeText(payload.estudante || '');
  const normalizedActivity = normalizeText(payload.atividade || payload.avaliacao || '');
  const normalizedSerie = normalizeText(payload.serie || '');
  const normalizedTurma = normalizeText(payload.turma || '');
  const normalizedBimestre = normalizeText(payload.bimestre || '');
  const normalizedType = normalizeText(payload.recuperacao ? 'recuperacao' : 'prova');
  const normalizedScore = normalizeText(payload.nota != null ? String(payload.nota) : '');
  const normalizedSheet = normalizeText(payload.sheetName || '');
  const normalizedCategory = normalizeText(payload.categoria || '');
